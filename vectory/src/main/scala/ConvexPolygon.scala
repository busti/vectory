...
abstract class ConvexPolygonLike {
  ...
  def includes(v: Vec2d): Boolean = edges.forall(_ leftOf v) // edges are ccw
  @inline def includes(l: Line): Boolean = includes(l.start) && includes(l.end)
  def intersectsMtd(that: ConvexPolygonLike): Option[Vec2d] = Algorithms.intersect2ConvexPolygonMtd(this, that)
  def intersects(that: Circle): Boolean = Algorithms.intersectCircleConvexPolygon(this, that)
  def intersectsMtd(that: Circle): Option[Vec2d] = Algorithms.intersectCircleConvexPolygonMtd(this, that, flip = false)
}
